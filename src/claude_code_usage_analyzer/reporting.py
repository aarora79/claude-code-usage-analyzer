...
def generate_quarto_report(
    ...
    """Generate a Quarto (.qmd) document from an analysis result.

    The body reuses the Markdown report so the two formats never drift. A YAML
    front matter block adds an HTML/PDF theme and table of contents. Render with
    ...
    Returns:
        The report as a single Quarto document string.
    """
    logger.info("Generating Quarto report...")

    generated_date = datetime.fromisoformat(analysis["metadata"]["generated_at"]).strftime(
        "%Y-%m-%d"
    )
    ...
    # Drop the leading H1 title line from the Markdown body; Quarto renders the
    # title from the front matter instead.
    markdown_body = generate_markdown_report(analysis)
    body_lines = markdown_body.split("\n")
